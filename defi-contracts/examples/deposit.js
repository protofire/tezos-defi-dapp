...
const madeDeposit = async () => {
    ...
    // Add values to table
    table.push([`Before deposit ${amount} ꜩ`, accountFaucetA, tzFormatter(beforeAccountBalance, 'tz'), tzFormatter(beforeDepositBalance, 'tz'), tzFormatter(beforePoolStorage.totalDeposits, 'tz'), tokenAmountInUnitsWithSymbol(beforeTokenBalance, 18, beforeTokenStorage.symbol)]);
    ...
    // Add values to table
    table.push([]);
    table.push([`After deposit ${amount} ꜩ`, accountFaucetA, tzFormatter(afterAccountBalance, 'tz'), tzFormatter(afterDepositBalance, 'tz'), tzFormatter(afterPoolStorage.totalDeposits, 'tz'), tokenAmountInUnitsWithSymbol(afterTokenBalance, 18, beforeTokenStorage.symbol)]);
